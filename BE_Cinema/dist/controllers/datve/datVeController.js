...
exports.lichSuGiaoDich = exports.datVe = void 0;
...
const datVe = async (req, res) => {
    try {
        ...
        const ghes = await prisma_1.default.ghe.findMany({
        ...
        // Calculate total
        const tongTien = ghes.reduce((sum, ghe) => sum + suatChieu.phim.giaCoBan + ghe.loaiGhe.phuPhi, 0);
        // Create payment
        const thanhToan = await prisma_1.default.thanhToan.create({
            data: {
                userId,
                tongTien,
                phuongThuc,
        ...
        // Create tickets
        const ves = await Promise.all(ghes.map((ghe) => prisma_1.default.ve.create({
            data: {
                userId,
                suatChieuId,
                gheId: ghe.id,
...
exports.datVe = datVe;
const lichSuGiaoDich = async (req, res) => {
    try {
        const userId = req.user?.userId;
        if (!userId) {
            res.status(401).json({ message: "Unauthorized" });
        ...
        const giaoDichs = await prisma_1.default.thanhToan.findMany({
            where: { userId },
            include: {
                ves: {
                    include: {
...
exports.lichSuGiaoDich = lichSuGiaoDich;
//# sourceMappingURL=datVeController.js.map